...
describe('Testa os controllers da rota /users', () => {
  ...
  describe('Testa se houve sucesso na requisição create', () => {
    before(() => {
      sinon.stub(UserService, 'create').resolves(mocks.createUserResolves);
      res.status = sinon.stub().returns(res);
      res.json = sinon.stub().returns(res);
      req.body = mocks.createUserRequest;
    });
  ...
  describe('Testa se houve rejeição na requisição create', () => {
    before(() => {
      sinon.stub(UserService, 'create').resolves(mocks.createUserResolves);
      sinon.stub(UserService, 'findUser').resolves(mocks.findUser)
      res.status = sinon.stub().returns(res);
      res.json = sinon.stub().returns(res);
      req.body = mocks.createUserRequest;
    });
